...
public void subdivide(PixelReader reader, double threshold, int maxDepth) {
    ...
    // Returns all leaf nodes whose variance exceeds the threshold (regions of interest)
    public List<QuadtreeNode> getInterestingLeaves(double threshold) {
        List<QuadtreeNode> result = new ArrayList<>(null);
        collectInteresting(threshold, result);
        return result;
    }

    private void collectInteresting(double threshold, List<QuadtreeNode> result) {
        if (isLeaf()) {
